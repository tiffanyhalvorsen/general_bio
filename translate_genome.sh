...
## This script translates a nucleotide genome assembly into its constituent protein sequences and makes a
## searchable database from those protein sequences for querying.

## Must run in directory with genome files (.fna). Genomes can also be in a subdirectory.
## Input files must be of the pattern "GC*genomic.fna" or, if metagenomes: "*_MAG_genomic.fna"
## Output will be created in working directory with the same accession prefix followed by '_prots.faa'

## Dependencies: prodigal, diamond.

## Usage: ./translate_genome.sh 
